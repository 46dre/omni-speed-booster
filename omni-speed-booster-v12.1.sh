cat << 'EOF' > /root/omni-speed-booster-v12.1.sh
#!/bin/bash
echo "🌌 MEMULAI OMNI SPEED BOOSTER V12.1 - GLOBAL INTERNET MENTOK 100% (Debian 13 Trixie) 🌌"
echo "💡 100% Network + DNS-over-HTTPS Cloudflare | Anti-error | No Hardware"

# =============================================
# 1. UPDATE & INSTALL (Minimal)
# =============================================
...
# Bersihkan file sysctl lama yang konflik
rm -f /etc/sysctl.d/99-omni-network-max.conf 2>/dev/null || true
rm -f /etc/sysctl.d/11-disable-ipv6.conf 2>/dev/null || true
...
echo "   - Semua error sudah dihilangkan"
echo "   - Reboot direkomendasikan: reboot"
EOF

chmod +x /root/omni-speed-booster-v12.1.sh && /root/omni-speed-booster-v12.1.sh
